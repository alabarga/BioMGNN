...
def compute_metrics(label, model, X, y):
    ...
    for metric in scoring:
        ...
        metrics_with_ci[metric] =  f'{mean_score:.3f} ({lower_bound:.2f} - {upper_bound:.2f})'
        # {
        ...
        result.append ( {
            'method': label,
            'metric': metric,
            'score': f'{mean_score} ({lower_bound}-{upper_bound})'
        })
...
class Study():
    ...
    def annotateGEO(self, modality, gpl_platform, id, name):
        soft_file = self.base_path / modality / '1.Annotation' / f'{gpl_platform}_family.soft.gz'
    ...
    def show_expression_mod(self, id, modality='mRNA', groups=None,**kwargs):
    ...
    def plot_clustered_heatmap(self, modality, groups=None):
        """
        Plots a heatmap with hierarchical clustering of rows in X and a colorbar according to classes in y.
